...
int blinkPin[6] = {13, 1, 2, 3, 4, 5};                // pin to blink led at each beat
int fade_pins[6] = {6, 1, 2, 3, 4, 5};                  // pin to do fancy classy fading blink at each beat
...
volatile int BPM[6];                   // int that holds raw Analog in 0. updated every 2mS
volatile int IBI[6] = {600, 0, 0, 0, 0, 0};             // int that holds the time interval between beats! Must be seeded! 
volatile int fade_leds_power[6] = {0, 0, 0, 0, 0, 0};                 // used to fade LED on with PWM on fadePin
volatile int NUM_OF_SENSORS = 1;
...
void loop(){
  ...
  for (int sensor=0; sensor<NUM_OF_SENSORS; ++sensor) { 
    if (QS[sensor]){     // A Heartbeat Was Found
          ...
          fade_leds_power[sensor] = 0;         // Makes the LED Fade Effect Happen
